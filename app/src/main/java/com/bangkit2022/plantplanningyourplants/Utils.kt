...
fun createFile(application : Application) : File {
    val externalMediaDirectory = application.externalMediaDirs.firstOrNull()?.let {
        File(it, "MyCamera").apply {
            mkdirs()
        }
    }

    val outputDirectory = if (externalMediaDirectory != null  &&  externalMediaDirectory.exists()) {
...
val timeStamp : String = SimpleDateFormat(FILE_DATE_FORMAT, Locale.US).format(System.currentTimeMillis())